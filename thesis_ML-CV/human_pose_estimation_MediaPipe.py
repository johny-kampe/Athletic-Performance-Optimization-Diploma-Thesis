...
def single_video_find_landmarks_human_pose_estimation(sheet_set_csv, set_csv, folder, name_csv):
    ...
    path = folder + 'DSC_0078_Trim.mp4'  # use any video that you like from any set
    ...
    while True:
        ...
        if results.pose_landmarks:
            ...
            for id, lm in enumerate(results.pose_landmarks.landmark):
                ...
                if id == 12 and count < 30:  # right shoulder -> insert it at the right column and row in the .csv
                    # print(f"Count={count}")
                    sheet_set_csv.write(rows12 + 1, 0, cx)
                    sheet_set_csv.write(rows12 + 1, 1, cy)
                    rows12 += 1
                elif id == 14 and count < 30:  # right elbow -> insert it at the right column and row in the .csv
                    # print(f"Count={count}")
                    sheet_set_csv.write(rows14 + 1, 2, cx)
                    sheet_set_csv.write(rows14 + 1, 3, cy)
                    rows14 += 1
                elif id == 16 and count < 30:  # right wrist -> insert it at the right column and row in the .csv
                    # print(f"Count={count}")
            ...
            if results.pose_landmarks:
                ...
                for id, lm in enumerate(results.pose_landmarks.landmark):
                    ...
                    if id == 12 and count < 30:  # right shoulder -> insert it at the right column and row in the .csv
                        ...
                        sheet_set_csv.write(rows14 + 1, 3, cy)
                        rows14 += 1
                        # print(f"14Count: {count}")

                    elif id == 16 and count < 30:  # right wrist -> insert it at the right column and row in the .csv
                        sheet_set_csv.write(rows16 + 1, 4, cx)
                        sheet_set_csv.write(rows16 + 1, 5, cy)
                        rows16 += 1
                        # print(f"16Count: {count}")
